...
Status ServerImp::call(ServerContext *context, const request *request,
    ...
    if (strcmp((request->command()).c_str(), "lookup") == 0) {
        ...
        cout << "server to lookup" << this->pathToFh.find(new_parent_path)->second << "\n";
...
//        struct fuse_file_info fi;
//        toFuseFileInfo(request->fi(), &fi);
//        fileInfo finfo = toProtoFileInfo(&fi);
//        int fh = finfo.fh();
//        string pth;
//        int op;
//        if (fh == 0) {
...
//        }
//        op = pwrite(fh, request->data().c_str(), request->size(), request->offset());
//        cout << "result of write, " << op << ", " << request->size() << ", " << request->offset() << "\n";
//        if (op == -1) {
//            response->set_success(-1);
//            return Status::OK;
//        }
//        response->set_status(0);
//        response->mutable_fi()->CopyFrom(finfo);
//        response->set_success(1);
        std::cout << "--------server_write request--------" << std::endl;
...
//    datastore.push_back(swr);
        ...
        string pth;
        if (fh == 0) {
            pth = this->root + this->findLookup(swr.fh);
            fh = open(pth.c_str(), O_WRONLY);
        }
        // std::cout << *a.data << "\n";
        int op = pwrite(fh, swr.data.c_str(), swr.size, swr.offset);
        std::cout << "result of write, " << op << ", " << swr.size << ", " << swr.offset << "\n";
        if (op == -1) {
            response->set_status(-1);
            std::cout << "write failed:  " << op << ", " << swr.size << ", " << swr.offset << "\n";
            return Status::OK;
        }

        response->set_status(0);
        response->set_datasize(request->size());
        response->mutable_fi()->CopyFrom(swr.fi);
        return Status::OK;
...
int ServerImp::addLookup(string path) {
    if (this->pathToFh.find(path) == this->pathToFh.end()) {
        ...
        this->fhcount++;
        cout << "the path: " << path << " at " << (this->fhcount - 1) << " in lookup function \n";
        this->storeMapping();
    }
